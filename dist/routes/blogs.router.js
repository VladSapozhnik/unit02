...
exports.blogsRouter.post(
  ...
  base_blog_validator_1.baseBlogValidator,
  input_validation_middleware_1.inputValidationMiddleware,
  (req, res) => {
    const randomId = (0, generate_id_1.generateId)();
    const newBlog = blogs_repository_1.blogsRepository.createBlog(
      req.body,
      randomId,
    );
    if (!newBlog) {
...
exports.blogsRouter.get('/:id', (req, res) => {
  const existBlog = blogs_repository_1.blogsRepository.getBlogById(
    req.params.id,
  );
  if (!existBlog) {
...
exports.blogsRouter.put(
  '/:id',
  super_admin_guard_middleware_1.superAdminGuardMiddleware,
  base_blog_validator_1.baseBlogValidator,
  input_validation_middleware_1.inputValidationMiddleware,
  (req, res) => {
    const isUpdated = blogs_repository_1.blogsRepository.updateBlog(
      req.params.id,
      req.body,
    );
    if (isUpdated) {
...
exports.blogsRouter.delete(
  '/:id',
  super_admin_guard_middleware_1.superAdminGuardMiddleware,
  (req, res) => {
    const isDelete = blogs_repository_1.blogsRepository.removeBlogById(
      req.params.id,
    );
    if (!isDelete) {
      res.sendStatus(http_status_1.HTTP_STATUS.NOT_FOUND_404);
