...
exports.postsRouter.get('/:id', (req, res) => {
  const existPost = posts_repository_1.postsRepository.getPostById(
    req.params.id,
  );
  if (!existPost) {
...
exports.postsRouter.put(
  '/:id',
  super_admin_guard_middleware_1.superAdminGuardMiddleware,
  base_post_validator_1.basePostValidator,
  input_validation_middleware_1.inputValidationMiddleware,
  (req, res) => {
    const isUpdatedPost = posts_repository_1.postsRepository.updatePost(
      req.params.id,
      req.body,
    );
    if (!isUpdatedPost) {
...
exports.postsRouter.delete(
  '/:id',
  super_admin_guard_middleware_1.superAdminGuardMiddleware,
  (req, res) => {
    const isRemove = posts_repository_1.postsRepository.removePost(
      req.params.id,
    );
    if (!isRemove) {
      res.sendStatus(http_status_1.HTTP_STATUS.NOT_FOUND_404);
